...
| Кластер поднимается с нуля по итоговому манифесту (без `pg_cron` в `shared_preload_libraries`) — Patroni на всех подах падает в бутстрапе, `post_init.sh` завершается кодом 3 | Встроенный `post_init.sh` Spilo всегда делает `CREATE EXTENSION pg_cron`, независимо от `preparedDatabases` — на первом бутстрапе `pg_cron` должен быть в `shared_preload_libraries`, убирать его можно только после того, как кластер уже поднялся |
