...
> **Грабли (исправлено в этом репозитории):** апстримный JSON содержал несколько независимых багов, из-за которых все панели показывали «No data» сразу после первого деплоя — переменная `namespace` имела буквальное значение вместо пустого `allValue`, `datasource` был захардкожен на чужой UID, а `instance` — на несуществующий хост из стороннего окружения, с фильтрами по точному равенству. Исправлено: `allValue` очищен, `datasource` указывает на реальный `VictoriaMetrics`, `instance` переведён на `multi: true` + `includeAll: true` (аналогично `namespace`), панели — на `instance=~"$instance"` (regex-match). Инстанс всегда резолвится в единственное значение (метрики отдаёт только текущий Leader — см. раздел 1), поэтому `All` по регэкспу эквивалентен точному совпадению, но не привязан к конкретному под-IP, который меняется при каждом передеплое.
