...
kubectl apply -f postgres/operator/postgres-pod-config.yaml -n postgres-operator
...
- `postgres-pod-config` (ConfigMap, ns `postgres-operator`) — общие для всех кластеров параметры WAL-G (`USE_WALG_BACKUP`, `BACKUP_SCHEDULE` и т.д.), передаются оператором как env всем подам.
...
- `configKubernetes.pod_environment_configmap`/`pod_environment_secret` — подключает созданные на шаге 4 конфигмап и секрет ко всем подам кластера.
...
`postgres/installations/postgres-cluster.yaml` — уже финальный манифест, включающий всё сразу:
...
- `postgres-metrics-svc.yaml` — headless Service, открывающий порты экспортёров (`9187` pg-exporter, `8008` Patroni, `9351` walg-exporter) по отдельному DNS-имени на каждый под;
- `vmservicescrape-postgres.yaml` — говорит VMAgent, откуда собирать метрики;
- `postgres-exporter-queries-cm.yaml` — кастомный SQL-запрос к `pg_extension`, чтобы видеть в Grafana реально установленные в базах расширения.
